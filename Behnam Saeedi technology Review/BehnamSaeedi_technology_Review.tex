...
This document explains some of the reasons to decisions made by Visual Flow team in order to create the WYSIWYG TensorFlow graphical user interface.
...
Finally, in programming, when the code grows large, there is a point in which after that a bug cannot be correctly addressed without introducing one plus epsilonÄù error to the code. \cite{criticalmass}
At that point the program cannot be corrected without rewriting it.

\subsection{Individual Project generation}
...
In our solution, Extensible Markup Language, C++‚Äôs Open Graphics Library and Python are few of our options in addressing this problem.
...
It needs to provide a graphical user interface for generating machine learning applications in python using the google'Äôs TensorFlow library for python. 
...
Our second solution is to create an executable for user'Äôs finished project. 
...
For this solution the graphical user interface provides a python script file with .pyÄù extension. 
...
Python does not compile the python script stored in Äú.py files, but rather interpret them. 
...
Since python is an interpreter the .pyÄù files will still be portable between different systems with different processors and operating systems. 
